...
export function ReelsSection({
  ...
  const [internalShorts, setInternalShorts] = useState<ShortVideo[]>(() => {
    try {
  ...
  const shorts = externalShorts !== undefined ? externalShorts : internalShorts;
  const setShorts = externalSetShorts !== undefined ? externalSetShorts : setInternalShorts;
  ...
  return (
    <div
      className="relative w-full h-full flex-1 min-h-0 bg-black flex flex-col items-center justify-center overflow-hidden select-none"
    ...
    >
      ...
      <header className="absolute top-0 left-0 right-0 z-40 px-4 pt-3 pb-6 bg-gradient-to-b from-black/80 via-black/40 to-transparent flex items-center justify-between pointer-events-auto">
        ...
        {/* Right: Search, Create Short & Sound Mute Toggle */}
        <div className="flex items-center space-x-2">
          {onOpenCreateShort && (
            <button
              onClick={onOpenCreateShort}
              className="px-3 py-1.5 rounded-full bg-gradient-to-r from-cyan-400 to-purple-600 text-zinc-950 font-bold text-xs hover:scale-105 transition cursor-pointer shadow-lg shadow-cyan-500/25 flex items-center gap-1.5"
      ...
      {!currentShort && (
        <div className="flex flex-col items-center justify-center text-center p-8 z-20">
          <div className="w-16 h-16 rounded-full bg-zinc-900/80 border border-zinc-800 flex items-center justify-center mb-4 text-zinc-500 shadow-xl">
            <Film className="w-8 h-8 text-cyan-400/80" />
          </div>
          <h3 className="text-lg font-bold text-zinc-200 mb-1">No Reels Yet</h3>
          <p className="text-sm text-zinc-500 max-w-xs mb-4">
            Be the first to post a short video clip, custom audio track, or nocturnal story.
          </p>
          {onOpenCreateShort && (
            <button
              onClick={onOpenCreateShort}
              className="px-5 py-2.5 bg-gradient-to-r from-cyan-400 to-purple-600 text-zinc-950 font-bold text-xs rounded-xl shadow-lg hover:scale-105 active:scale-95 transition flex items-center gap-2 cursor-pointer shadow-cyan-500/25"
              id="reels-empty-create-short-btn"
            >
